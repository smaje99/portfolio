...
| Proyectos públicos principales: TrazalITA y EpicrisisIA | Demostrar en `/projects`, `/en/projects` y destacados de home que existen sistemas en desarrollo orientados a procesos, datos y operación. | `critico` / `existente con validación` | `src/data/projects.ts`, colecciones editoriales, `ProjectsGrid.astro` y `PortfolioPage.astro`; **conservar** como inventario base, sin tratarlos aún como casos de estudio. | Validación manual de alcance, estado y wording de cada afirmación pública; la evidencia profunda queda para Fase 3. | `Mixto` | O2-KR3, O2-KR4, O4-KR1; `BLG-F3-S05-01`, `BLG-F3-S05-03`. |
